...
class DirectoryDigester(numThreads: Int)(implicit concurrent: Concurrent[IO]) {
  ...
  def digestFor(directory: Path): IO[SHA256Digest] =
    for {
      ...
      directoryDigest: SHA256Digest <- Stream.emits[IO, Path](allFilesInOrder)
  ...
  private def digestsFor(paths: Chunk[Path]): IO[Chunk[SHA256Digest]] = IO {
  ...
  private object CollectorIntoDigest extends Collector[Chunk[SHA256Digest]] {
    ...
    override def newBuilder: Collector.Builder[Chunk[SHA256Digest], SHA256Digest] = new Collector.Builder[Chunk[SHA256Digest], SHA256Digest] {
